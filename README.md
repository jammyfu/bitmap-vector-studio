...
## 🎯 Less is More 界面理念（v3.0 UI重构）

Bitmap Vector Studio v3.0 对桌面应用和 Streamlit GUI 进行了全面的「Less is More」界面重构，核心目标是在不牺牲功能的前提下，将界面复杂度降至最低。

- 🤖 **智能默认**：上传图片后自动分析并推荐最佳预设，高置信度时自动应用
- 🎨 **单栏聚焦**：中央预览画布 + 底部控制带，减少视觉跳跃，让用户专注于创作
- 📦 **渐进披露**：仅显示3个核心参数，高级功能默认折叠在底部抽屉中
- 🔍 **Command Palette**：`Cmd/Ctrl+K` 统一搜索所有功能、预设、设置和文件
- 📱 **响应式自适应**：自动适配桌面/平板/小窗口，紧凑模式下控制带自动收拢
...
```
┌─────────────────────────────────────────────────────────────┐
│  Logo    🔍 Command Palette    设置  主题                    │  ← 顶部栏
├─────────────────────────────────────────────────────────────┤
│                                                             │
│                    中央预览画布                               │
│                                                             │
...
| 命令 | 说明 |
|---|---|
| `vector-studio convert` | **v3.0 推荐** 统一转换入口，自动识别单图/批量/生成 |
| `vector-studio quick` | **v3.0 推荐** 一键快速转换，使用智能默认 |
| `vector-studio presets` | 查看所有内置预设 |
| `vector-studio trace` | 单张图片转换（高级） |
| `vector-studio batch` | 批量转换文件夹（高级） |
...
| `vector-studio workspace list` | **v1.1** 列出所有工作区 |
...
`convert` 是 v3.0 统一的转换入口，自动识别输入类型并选择最佳处理路径：

```bash
# 单图转换（自动推荐预设）
vector-studio convert input.png

# 指定预设
vector-studio convert input.png --preset logo

# 批量转换文件夹
vector-studio convert ./inputs ./outputs --preset poster

# 完整参数控制
vector-studio convert photo.jpg --preset photo --color-precision 7 --export-pdf
```

| 选项 | 简写 | 类型 | 默认值 | 说明 |
|---|---|---|---|---|
| `--preset` | `-p` | str | `auto` | 预设名称，`auto` 时自动推荐 |
| `--output` | `-o` | Path | 同目录 `.svg` | 输出路径 |
| `--recursive` | `-r` | bool | `False` | 递归扫描子文件夹（批量模式） |
| `--export-pdf` | | bool | `False` | 同时导出 PDF |
| `--export-png` | | bool | `False` | 同时导出 PNG 预览 |
| `--quiet` | `-q` | bool | `False` | 静默模式，仅输出错误 |
| `--verbose` | `-v` | bool | `False` | 详细模式，显示完整处理日志 |

### `quick` 命令（v3.0 推荐）

`quick` 是一键快速转换，使用智能默认，零配置即可获得最佳结果：

```bash
# 最简用法：自动分析、自动推荐、自动转换
vector-studio quick input.png

# 批量快速转换
vector-studio quick ./inputs/*.png

# 指定输出目录
vector-studio quick ./inputs --output-dir ./outputs
```

| 选项 | 简写 | 类型 | 默认值 | 说明 |
|---|---|---|---|---|
| `--output-dir` | `-o` | Path | 同目录 | 输出目录 |
| `--yes` | `-y` | bool | `False` | 自动确认所有提示 |
