...
# 健康检查
print(client.health())  # {'status': 'ok', 'version': '1.2.0'}
```
...
**当前阶段（v1.2.0）**：稳定性与生态扩展 — 多矢量化引擎（VTracer/Potrace/AutoTrace）、插件 SDK 完善（验证器/脚手架/调试器/文档生成）、云端同步预览（分享链接+QR码）、社区工具链（预设验证/贡献指南/发布说明）、测试覆盖率 90%+。

**下一阶段（v2.0）**：AI 原生与实时协作 — 本地 AI 模型推理、实时协作编辑、云端预设与插件市场、矢量动画导出、智能批处理工作流、跨设备同步。

**长期目标（v2.0）**：AI 原生与实时协作 — 本地 AI 模型推理、实时协作编辑、云端预设与插件市场、矢量动画导出、智能批处理工作流、跨设备同步。
