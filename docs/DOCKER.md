...
Bitmap Vector Studio v2.0 提供多阶段 Dockerfile 和 docker-compose.yml，支持以 API 服务或 CLI 工具两种方式运行。
...
docker buildx build \
  ...
  -t jammyfu/bitmap-vector-studio:2.0.0 \
...
docker buildx build \
  --target cli \
  --platform linux/amd64,linux/arm64 \
  -t jammyfu/bitmap-vector-studio:2.0.0-cli \
  --push .
```
