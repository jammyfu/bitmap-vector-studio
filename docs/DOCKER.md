...
Bitmap Vector Studio v1.1 提供多阶段 Dockerfile 和 docker-compose.yml，支持以 API 服务或 CLI 工具两种方式运行。

> **v1.1 新增**：桌面应用已作为首选使用方式，Docker 部署适合服务器端 API 服务和自动化流水线场景。详见 [docs/DESKTOP.md](DESKTOP.md)。
...
docker buildx build \
  ...
  -t jammyfu/bitmap-vector-studio:1.1.0 \
...
docker buildx build \
  --target cli \
  --platform linux/amd64,linux/arm64 \
  -t jammyfu/bitmap-vector-studio:1.1.0-cli \
  --push .
```
