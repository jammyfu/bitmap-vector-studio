...
v0.3 新增的智能预设推荐、背景透明检测等功能依赖 NumPy 进行图像特征分析：

```bash
pip install -e ".[smart]"
```

这会安装 `numpy>=1.24`。不安装时智能功能仍可用，但分析精度和速度会下降。
...
| 功能 | 必需依赖 | 可选依赖 | 说明 |
|---|---|---|---|
| 核心矢量化 | `vtracer`, `Pillow` | — | 所有功能的基础 |
| 智能预设推荐 | `Pillow` | `numpy` | NumPy 提升颜色聚类和边缘分析精度 |
| 智能背景透明 | `Pillow` | `numpy` | NumPy 加速背景色检测和掩码生成 |
| 图像增强 | `Pillow` | — | 纯 Pillow 实现，无需 OpenCV |
| SVG 深度优化 | — | — | 纯标准库实现 |
| 参数搜索 | `vtracer`, `Pillow` | `numpy` | 批量试跑需要核心依赖 |
| 批量任务队列 | `vtracer`, `Pillow` | — | 并发转换需要核心依赖 |
| PDF/PNG 导出 | `cairosvg` | — | 已包含在默认依赖中 |
| EPS 导出 | — | Inkscape | 需系统级安装 Inkscape |

> **提示**：若仅使用核心转换和批量功能，无需额外安装。若希望获得最佳智能分析体验，建议安装 `pip install -e ".[smart]"`。
